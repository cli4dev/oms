...
func NewNotifyHandler(c component.IContainer) *NotifyHandler {
	return &NotifyHandler{
		c:    c,
		nLib: notify.NewInvoiceNoitfyOms(c),
	}
}
...
func (n *NotifyHandler) Handle(ctx *context.Context) (r interface{}) {
	...
	defer lcs.New(ctx, "开票结果通知", types.GetString(ctx.Request.GetString("order_id")), types.GetString(invoiceID)).Start("开票结果通知").End(err)
	if err != nil {
		return err
	}
