...
and t.refund_id=if(@refund_id=0,t.refund_id,@refund_id)
...
sum(if(t.request_type= 1, 1, -1) * t.buy_send_num) last_buy_num,
sum(if(t.request_type= 1, 1, -1) * t.activity_send_num) last_activity_num
from jf_fd_order_record t
where t.order_id = @order_id
