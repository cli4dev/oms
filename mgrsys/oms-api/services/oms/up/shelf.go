...
type ShelfHandler struct {
	...
	ShelfLib  up.IDbShelf
}

//NewShelfHandler 创建上游货架对象
func NewShelfHandler(container component.IContainer) (u *ShelfHandler) {
	return &ShelfHandler{
		container: container,
		ShelfLib:  up.NewDbShelf(container),
	}
}
...
func (u *ShelfHandler) PostHandle(ctx *context.Context) (r interface{}) {
	...
	ctx.Log.Info("--------添加上游货架数据--------")
	ctx.Log.Info("1.参数校验")

	var input up.CreateShelf
...
//QueryHandle  获取上游货架数据列表
func (u *ShelfHandler) QueryHandle(ctx *context.Context) (r interface{}) {

	ctx.Log.Info("--------获取上游货架数据列表--------")
	ctx.Log.Info("1.参数校验")

	var input up.QueryShelf
	if err := ctx.Request.Bind(&input); err != nil {
		return context.NewError(context.ERR_NOT_ACCEPTABLE, err)
	}
...
func (u *ShelfHandler) PutHandle(ctx *context.Context) (r interface{}) {
	...
	ctx.Log.Info("--------更新上游货架数据--------")
	ctx.Log.Info("1.参数校验")

	var input up.UpdateShelf
	if err := ctx.Request.Bind(&input); err != nil {
		return context.NewError(context.ERR_NOT_ACCEPTABLE, err)
	}
