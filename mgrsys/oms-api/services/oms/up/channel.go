...
//ChannelHandler 上游渠道接口
type ChannelHandler struct {
	container  component.IContainer
	ChannelLib up.IDbChannel
}

//NewChannelHandler 创建上游渠道对象
func NewChannelHandler(container component.IContainer) (u *ChannelHandler) {
	return &ChannelHandler{
		container:  container,
		ChannelLib: up.NewDbChannel(container),
	}
}
...
func (u *ChannelHandler) PostHandle(ctx *context.Context) (r interface{}) {
	...
	ctx.Log.Info("--------添加上游渠道数据--------")
	ctx.Log.Info("1.参数校验")

	var input up.CreateChannel
...
//QueryHandle  获取上游渠道数据列表
func (u *ChannelHandler) QueryHandle(ctx *context.Context) (r interface{}) {

	ctx.Log.Info("--------获取上游渠道数据列表--------")
	ctx.Log.Info("1.参数校验")

	var input up.QueryChannel
	if err := ctx.Request.Bind(&input); err != nil {
		return context.NewError(context.ERR_NOT_ACCEPTABLE, err)
	}
...
func (u *ChannelHandler) PutHandle(ctx *context.Context) (r interface{}) {
	...
	ctx.Log.Info("--------更新上游渠道数据--------")
	ctx.Log.Info("1.参数校验")

	var input up.UpdateChannel
	if err := ctx.Request.Bind(&input); err != nil {
		return context.NewError(context.ERR_NOT_ACCEPTABLE, err)
	}
...
func (u *ChannelHandler) SetSecretHandle(ctx *context.Context) (r interface{}) {
	...
	ctx.Log.Info("--------设置秘钥--------")
	ctx.Log.Info("1.参数校验")
	if err := ctx.Request.Check("secret", "channel_no"); err != nil {
		return context.NewError(context.ERR_NOT_ACCEPTABLE, err)
	}

	ctx.Log.Info("2.执行操作")
	err := u.ChannelLib.SetSecret(ctx.Request.GetString("secret"), ctx.Request.GetString("channel_no"))
	if err != nil {
		return context.NewError(context.ERR_NOT_IMPLEMENTED, err)
	}
