      ...
      <el-form ref="form" :inline="true" class="form-inline pull-left">
        ...
        <el-form-item>
          <el-select
            ...
            clearable
          >
            <el-option value label="全部上游渠道"></el-option>
            <el-option
      ...
      <table class="table table-striped m-b-none th-bg">
        <thead>
          <tr>
            ...
            <th style="text-align:center">成本金额</th>
            <th style="text-align:center">成本扣款</th>
          </tr>
        </thead>
        ...
        <tbody class="table-border" else>
          <tr v-for="(item, index) in dataList" :key="index">
            ...
            <td style="text-align:center">{{item.delivery_count |AmountFilter}}</td>
            ...
            <td style="text-align:center">{{item.success_up_commi |AmountFilter}}</td>
          </tr>

          <tr>
            <td style="text-align:center;padding: 14px 15px" colspan="1">总计</td>
            <td style="text-align:center">{{totaldata.delivery_count |AmountFilter}}</td>
            ...
            <td style="text-align:center">{{totaldata.success_up_commi |AmountFilter}}</td>
          </tr>
        </tbody>
      </table>
    </div>
...
export default {
  ...
  methods: {
    ...
    Export() {
      ...
      oReq.open(
        "POST",
        process.env.VUE_APP_API_URL + "/oms/report/upchannel/export",
        true
      );
      ...
      oReq.onload = function(oEvent) {
        ...
        elink.download = "上游渠道统计报表.xlsx"; //xlsx    因为后台输入是scv格式，用xlsx显示的不理想
    ...
    QueryDataList() {
      ...
      this.$http
        .get("/oms/report/upchannel/query", this.params)
        .then(response => {
