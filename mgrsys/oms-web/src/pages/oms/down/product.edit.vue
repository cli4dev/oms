<template>
  <el-dialog title="编辑下游商品" width="45%" @closed="closed" :visible.sync="dialogFormVisible">
    <el-form :model="editData" :inline="true" :rules="rules" ref="editForm" label-width="140px">
      <el-form-item label="货架编号:" prop="shelf_id">
        <el-select
          placeholder="---请选择---"
          clearable
      ...
      <el-form-item label="是否允许拆单:" prop="can_split_order">
        <el-select
          placeholder="---请选择---"
          clearable
          v-model="editData.can_split_order"
          style="width: 100%;"
        >
          <el-option
...
<el-form-item label="拆单面值" prop="split_order_face" v-if="editData.can_split_order=='0'">
        <el-input clearable v-model="editData.split_order_face" placeholder="请输入拆单面值"></el-input>
      </el-form-item>
      ...
      <el-form-item label="单次最大购买数量" prop="limit_count">
        <el-input clearable v-model="editData.limit_count" placeholder="请输入单次最大购买数量"></el-input>
      </el-form-item>
      ...
      <el-form-item label="手续费折扣" prop="payment_fee_discount">
        <el-input clearable v-model="editData.payment_fee_discount" placeholder="请输入手续费折扣"></el-input>
      </el-form-item>
      ...
      <el-form-item label="销售折扣" prop="sell_discount">
        <el-input clearable v-model="editData.sell_discount" placeholder="请输入销售折扣"></el-input>
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    setCityNO(value) {
		...
		this.$http
        .post("/oms/canton/info/getdictionarybyprovince", {grade: 2,canton_code:value})
        .then(res => {
          this.CityList = res
        })
        .catch(err => {
    ...
    edit() {
      this.editData.create_time = this.DateConvert(
        "yyyy-MM-dd hh:mm:ss",
        this.editData.create_time
      );
       if (this.editData.can_split_order == "1") {
            this.editData.split_order_face = "0";
          }
      this.$http
        .put("/oms/down/product", this.editData)
        .then(res => {
          this.$message({
            type: "success",
            message: "修改成功!"
