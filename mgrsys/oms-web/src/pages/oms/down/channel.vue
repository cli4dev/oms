  ...
  <div class="panel panel-default">
    ...
    <el-scrollbar style="height:100%">
      <el-table :data="tableData" border style="width: 100%">
        <el-table-column prop="channel_no" label="编号" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.channel_no | EllipsisFilter(20)}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="channel_name" label="名称" align="center">
          <template slot-scope="scope">
        ...
        <el-table-column label="操作" align="center">
          <template slot-scope="scope">
            ...
            <el-button type="text" size="small" @click="setSecret(scope.row.channel_no)">设置秘钥</el-button>
...
export default {
  ...
  methods: {
    setSecret(val) {
      this.$confirm("此操作将生成秘钥, 是否继续?", "提示", {
        confirmButtonText: "确定",
        cancelButtonText: "取消",
        type: "warning"
      }).then(() => {
        this.$http
          .get("/oms/down/channel/setsecret", {channel_no:val})
          .then(res => {
                this.$message({
                  type:"info",
                  message:"生成秘钥成功"
    ...
    query() {
      ...
      this.$http
        .get("/oms/down/channel/query", this.queryData)
        .then(res => {
    ...
    detailShow(val) {
      var data = {
        getpath: "/oms/down/channel",
        channel_no: val.channel_no
      };
      this.$emit(
        "addTab",
        "详情",
        "/oms/down/channel.view",
        data,
        false
      );
    },
