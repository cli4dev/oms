<template>
  <!-- Add Form -->
  <el-dialog title="添加下游渠道" width="20%" :visible.sync="dialogAddVisible">
    <el-form :model="addData"  :rules="rules" ref="addForm" label-width="100px">
      <el-form-item label="编号" prop="channel_no">
				<el-input clearable v-model="addData.channel_no" placeholder="请输入编号">
				</el-input>
      </el-form-item>
      
      <el-form-item label="名称" prop="channel_name">
				<el-input clearable v-model="addData.channel_name" placeholder="请输入名称">
				</el-input>
      </el-form-item>
	...
	methods: {
		...
		add(formName) {
			this.$refs[formName].validate((valid) => {
				if (valid) {
					this.$http.post("/oms/down/channel", this.addData)
						.then(res => {
							this.$refs[formName].resetFields()
							this.dialogAddVisible = false
							this.refresh()
