  ...
  <div class="panel panel-default">
    ...
    <div class="panel-body">
      <el-form ref="form" :inline="true" class="form-inline pull-left">
        ...
        <el-form-item>
          <el-select
            size="medium"
            v-model="queryData.up_refund_status"
            class="input-cos"
            placeholder="请选择退款状态"
          >
            <el-option value label="全部"></el-option>
            <el-option
              v-for="(item, index) in upRefundStatus"
              :key="index"
              :value="item.value"
              :label="item.name"
            ></el-option>
    ...
    <el-scrollbar style="height:100%">
      <el-table :data="tableData" border style="width: 100%">
        ...
        <el-table-column prop="return_id" label="退货编号" align="center"></el-table-column>
        ...
        <el-table-column prop="down_channel_no" label="下游渠道" min-width="120px" align="center" >
          <template slot-scope="scope">
            <span>{{scope.row.down_channel_no | EnumFilter("DownChannelNo")}}</span>
          </template>
        </el-table-column>
       
        <el-table-column prop="up_channel_no" label="上游渠道" min-width="120px" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.up_channel_no | EnumFilter("UpChannelNo")}}</span>
          </template>
        </el-table-column>
        

        <el-table-column prop="line_id" label="产品线" align="center" min-width="120px">
          <template slot-scope="scope">
        ...
        <el-table-column prop="return_face" label="面值" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.return_face | AmountFilter}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="return_num" label="数量" align="center"></el-table-column>
        <el-table-column prop="return_total_face" label="总面值" align="center">
          <template slot-scope="scope">
            <span>{{scope.row.return_total_face | AmountFilter}}</span>
          </template>
        </el-table-column>

        <el-table-column prop="up_refund_status" label="退款状态" align="center">
          <template slot-scope="scope">
            <span  :class="generalStatusClassFilter(scope.row.up_refund_status)">{{scope.row.up_refund_status | EnumFilter("UpRefundStatus")}}</span>
          </template>
        </el-table-column>
        <el-table-column prop="return_status" label="退货状态" align="center">
          <template slot-scope="scope">
            <span  :class="generalStatusClassFilter(scope.row.return_status)">{{scope.row.return_status | EnumFilter("ReturnStatus")}}</span>
          </template>
        </el-table-column>
...
export default {
  ...
  data() {
    return {
      ...
      provinceNo:this.EnumUtility.Get("ProvinceNo",{grade:1},"/oms/canton/info/getlist"),
      ...
      returnStatus: this.EnumUtility.Get("ReturnStatus"),
      dtCreateTime: this.DateConvert("yyyy-MM-dd 00:00:00", new Date()),
      upChannelNo: this.EnumUtility.Get(
        "UpChannelNo",
        {},
        "/oms/up/channel/getdictionary"
      ),
      upRefundStatus: this.EnumUtility.Get("UpRefundStatus"),
      tableData: []
    };
  },
  created() {},
  ...
  methods: {
    ...
    query(value) {
      ...
      this.$http
        .get("/oms/refund/up/return/query", this.queryData)
        .then(res => {
