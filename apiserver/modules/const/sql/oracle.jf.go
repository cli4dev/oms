...
(@id,
 ...
 @over_time)
`
const GetNewJFPreOrderID = `select seq_jf_pre_order_id.nextval from dual`
...
const GetNewFDRequestID = `select seq_jf_fd_request_id.nextval from dual
`
...
const GetNewFDOrderID = `select seq_jf_fd_id.nextval from dual`
...
and t.has_cancel = 1
`
const CancelPreOrderQuery = `select t.pre_order_id, t.user_no,to_char(t.create_time,'yyyy-mm-dd hh24:mi:ss') create_time
from jf_pre_order_info t
where t.down_channel_no = @channel_no
 and t.pre_request_no = @pre_request_no
 and t.has_cancel = 1
...
where t.create_time > to_date(@create_time, 'yyyy-mm-dd hh24:mi:ss')
