...
SELECT round(t.sell_amount/t.num,2) sell_amount,
       if(round(t.sell_amount/t.num,1) = round(@amount,1),'true','false') can_sell_amount,
       t.order_id,
       t.invoice_type,
       t.down_channel_no,
       t.down_product_id,
       ...
       if(round(t.point_num * 0.01/t.num,2) = @deduct_amount,'true','false') can_deduct_amount,
       IF(DATE_FORMAT(DATE_ADD(t.create_time, INTERVAL 1 MONTH),'%y-%m') >= DATE_FORMAT(NOW(),'%y-%m'),'true','false') can_invoice,
       p.invoice_obj_type
